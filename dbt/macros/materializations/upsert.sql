{% materialization upsert, adapter='trino' %}
    ...
    {{ log('[DEBUG] Starting UPSERT materialization for model: ' ~ model.name, info=false) }}
    ...
    {% if existing_relation and check_full_refresh() is false %}
        {% call statement('main') %}
            insert into {{ target_relation }}
                with changed as (
                    select
                        {% for c in primary_key %} "{{ c }}" {{ ',' if not loop.last }} {% endfor %}
                    ...
                    {% if 'is_active_flg' in sys_columns %} where is_active_flg = 1 {% endif %}
                )
                ...
                select
                    ...
                    {% endif %}
                from changed c
                join {{ source_relation }} src using ({% for c in primary_key %} "{{ c }}" {{ ',' if not loop.last }} {% endfor %})
                ;
                -- delete duplicates rows
                delete from {{ target_relation }}
                where (loaded_dttm, {% for c in primary_key %} "{{ c }}" {{ ',' if not loop.last }} {% endfor %}) in (
                    select
                        loaded_dttm,
                        {% for c in primary_key %} "{{ c }}" {{ ',' if not loop.last }} {% endfor %}
                    from (
                        select
                            loaded_dttm
                            , {% for c in primary_key %} "{{ c }}" {{ ',' if not loop.last }} {% endfor %}
                            , row_number() over (partition by {% for c in primary_key %} "{{ c }}" {{ ',' if not loop.last }} {% endfor %} order by loaded_dttm desc) as rn
                        from {{ target_relation }})
                    where rn != 1
                );

        {% endcall %}
        {{ log('[DEBUG] Upsert completed successfully: ' ~ target_relation, info=false) }}
    {% else %}
        {% call statement('main') %}
