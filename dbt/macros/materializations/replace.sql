{% materialization replace, adapter='trino' %}
    ...
    {% set min_replace_dt = get_min_value(column='dt', table_nm=source_relation) %}
    {% set max_replace_dt = get_max_value(column='dt', table_nm=source_relation) %}
    ...
    {{ log('[DEBUG] Starting REPLACE materialization for model: ' ~ model.name, info=false) }}
    ...
    {% if min_replace_dt is not none and max_replace_dt is not none %}
    ...
    {{ log('[DEBUG] Replacing period from: ' ~ min_replace_dt ~ ', replacing period to: ' ~ max_replace_dt, info=true) }}

    -- deleting replacing dates
    {% if existing_source_data %}
        {% call statement('main') %}
            delete from {{ target_relation }}
            where date'{{ min_replace_dt }}' <= dt and dt < date'{{ max_replace_dt }}' + interval '1' day
            ;
        {% endcall %}
    {% endif %}
    ...
    {% if existing_relation and check_full_refresh() is false %}
        {% call statement('main') %}
            ...
            ;
        {% endcall %}
        {{ log('[DEBUG] Replace completed successfully: ' ~ target_relation, info=false) }}
    {% else %}
        {% call statement('main') %}
